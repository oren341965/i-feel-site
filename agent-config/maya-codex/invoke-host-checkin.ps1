...
$reporterPath = Join-Path $userProfileRoot '.codex\skills\management-system-telemetry\scripts\report-host-checkin.mjs'
$reporterExitCode = 1
...
if (-not (Test-Path -LiteralPath $reporterPath -PathType Leaf)) {
    throw 'The managed I Feel host check-in reporter is not installed.'
}
