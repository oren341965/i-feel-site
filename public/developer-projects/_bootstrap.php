...
/**
 * Secure resident access for I Feel developer projects.
...
const ESP_SESSION_NAME = 'ifeel_dev_access';
const ESP_ACCESS_TTL = 7200;
const ESP_OTP_TTL = 600;
const ESP_OTP_RESEND_SECONDS = 60;
const ESP_OTP_HOURLY_LIMIT = 5;
const ESP_CSRF_COOKIE = 'ifeel_dev_csrf';
const ESP_OTP_COOKIE = 'ifeel_dev_otp';
...
{
    $slugs = [
        'group_mm1hqj4x' => 'mandelblat-even-ezra',
        'group_mm08ysw7' => 'zikhron-yaakov-13',
        'group_mm1883d7' => 'bnei-dan-8',
        'group_mm15570j' => 'even-shaprut',
        'group_mm159pb3' => 'harishonim-15',
        'group_mm1427xh' => 'reading-22-24',
        'group_mm13pm3m' => 'alterman-10-12',
        'group_mm074xmd' => 'amoraim',
        'group_mm13gr21' => 'sheshet-hayamim',
        'group_mm13ry3r' => 'louis-marshall',
        'group_mm12r532' => 'top-gan-hazetim',
        'group_mkxjpx0e' => 'alterman-6-8',
        'group_mky0w0qd' => 'sde-boker-hachnasat-30',
        'group_mky0hwdd' => 'weizmann-11-13',
        'group_mkxjwm7s' => 'antokolsky-15-17',
        'group_mm12hmj3' => 'nofei-shemesh',
...
function esp_user_has_project(array $user, string $groupId): bool
{
    ...
    foreach (($user['projects'] ?? []) as $project) {
        ...
        if ($pid !== '' && esp_project_slug($pid) === $wantedSlug) return true;
...
{
    return [
        'group_mm1hqj4x' => 'מנדלבלט/אבן עזרא',
        'group_mm08ysw7' => 'זכרון יעקב 13',
        'group_mm1883d7' => 'בנין דן 8',
        'group_mm15570j' => 'אבן שפרוט',
        'group_mm159pb3' => 'הראשונים 15',
        'group_mm1427xh' => 'רידינג 22-24',
        'group_mm13pm3m' => 'אלתרמן 10-12',
        'group_mm074xmd' => 'אמוראים',
        'group_mm13gr21' => 'ששת הימים',
        'group_mm13ry3r' => 'לואי מרשל',
        'group_mm12r532' => 'טופ גן- הזיתים',
        'group_mkxjpx0e' => 'אלתרמן 6-8',
        'group_mky0w0qd' => 'שדה בוקר + הכנסת 30, גבעתיים',
        'group_mky0hwdd' => 'וייצמן 11-13',
        'group_mkxjwm7s' => 'אנטוקולסקי 15-17',
        'group_mm12hmj3' => 'נופי שמש',
...
function esp_resident_profile(string $email): ?array
{
    ...
    $groups = esp_project_groups();
    $query = <<<'GRAPHQL'
query DeveloperProjectResidents($boardIds: [ID!], $groupIds: [String]) {
  boards(ids: $boardIds) {
    groups(ids: $groupIds) {
      id
      title
      items_page(limit: 500) {
        cursor
        items { id name column_values(ids: ["numbers21","text8","_____3","location7"]) { id text value } }
      }
    }
  }
}
GRAPHQL;
    ...
    foreach (($response['data']['boards'][0]['groups'] ?? []) as $group) {
        ...
        $items = is_array($group['items_page']['items'] ?? null) ? $group['items_page']['items'] : [];
        foreach ($items as $item) {
            ...
            $slug = esp_project_slug($gid);
            $foundProjects[$slug !== '' ? $slug : $gid] = $project;
            if ($best === null) $best = $profile;
        }
    }
    if ($best === null) { $cache[$email]=null; return null; }
    $best['projects'] = array_values($foundProjects);
    $cache[$email] = $best;
    return $best;
}
...
function esp_send_code(array $profile): bool
{
    ...
    $subjectText = 'קוד כניסה לאזור לקוחות פרויקטי I Feel';
    $subject = '=?UTF-8?B?' . base64_encode($subjectText) . '?=';
    $body = "שלום,\n\nקוד הכניסה שלך לאזור לקוחות פרויקטי I Feel הוא: {$code}\n\nהקוד תקף ל-10 דקות ולשימוש חד פעמי.\nאם לא ביקשת את הקוד, אפשר להתעלם מהודעה זו.\n\nI Feel מערכות בע\"מ\nמשרד 03-508-9553";
    $headers = [
