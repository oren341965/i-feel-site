...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    try {
        ...
        $eventId = mcohome_resolve_event_id(mcohome_post_value('client_event_id', 80));
        if (mcohome_load_record($eventId) !== null) {
            throw new RuntimeException('הדיווח כבר נשמר. מספר אירוע: ' . $eventId);
        }
        ...
        $createdRecord = mcohome_finalize_record($record);
        portal_audit('mcohome_fault_submitted', [
            'event_id' => $createdRecord['eventId'],
            'employee_hash' => hash('sha256', $verifiedEmail),
            'device_type' => $createdRecord['deviceType'],
            'fault_type' => $createdRecord['faultType'],
...
header("Content-Security-Policy: default-src 'none'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'; font-src 'self'; connect-src 'self'; manifest-src 'self'; worker-src 'self'; form-action 'self'; base-uri 'none'; frame-ancestors 'none'; object-src 'none'");
...
<section class="panel mcohome-panel">
    <div class="section-heading">
        <div>
            <h1>אפליקציית תקלות MCOHome</h1>
            <p>אפשר למלא גם ללא אינטרנט. הדיווח נשמר במכשיר ומסתנכרן אוטומטית כשהחיבור חוזר.</p>
        </div>
        <button type="button" class="button button--secondary" id="mcohome-install">התקנת האפליקציה</button>
    </div>

    <div id="mcohome-app-status" class="alert alert--info" hidden></div>
    <p class="form-note"><strong>ממתינים לשליחה מהמכשיר: <span id="mcohome-queue-count">0</span></strong></p>
    ...
    <form method="post" enctype="multipart/form-data" class="stack-form" id="mcohome-fault-form">
        ...
        <input type="hidden" name="client_event_id" value="">

        <div class="mcohome-quick-grid">
            <label><span>לקוח / פרויקט</span><input type="text" name="project" value="<?= portal_h(mcohome_post_value('project', 200)) ?>" maxlength="200" placeholder="שם הלקוח או הפרויקט" required></label>
            ...
            <label><span>דגם / מק״ט</span><input type="text" name="model" value="<?= portal_h(mcohome_post_value('model', 150)) ?>" maxlength="150" required></label>
        ...
        <fieldset class="mcohome-fieldset">
            ...
            <div class="mcohome-chip-grid"><?php foreach ($options['deviceTypes'] as $item): ?><label class="mcohome-chip"><input type="radio" name="device_type" value="<?= portal_h($item) ?>" <?= mcohome_post_value('device_type', 80) === $item ? 'checked' : '' ?> required><span><?= portal_h($item) ?></span></label><?php endforeach; ?></div>
        </fieldset>
        ...
        <label class="mcohome-media-box">
            <span>4. סרטון הסבר עד דקה או תמונות</span>
            <input type="file" name="media[]" id="mcohome-media" accept="image/*,video/*" capture="environment" multiple>
            <small>עד 6 קבצים, עד 140MB לקובץ. סרטון עד דקה. בהיעדר אינטרנט הסרטון נשמר במכשיר ומסתנכרן בהמשך.</small>
        ...
        <details class="mcohome-details">
            ...
            <div class="form-grid">
                ...
                <label><span>בדיקת Z-Wave</span><input type="text" name="zwave_check" value="<?= portal_h(mcohome_post_value('zwave_check', 300)) ?>" maxlength="300"></label>
                <label class="form-grid__wide"><span>עומס / דרייבר / מנוע</span><input type="text" name="load_context" value="<?= portal_h(mcohome_post_value('load_context', 400)) ?>" maxlength="400"></label>
        ...
        <button type="submit" class="button button--primary button--wide mcohome-submit" id="mcohome-submit">שמירת ושליחת התקלה</button>
        <p class="form-note">כל תקלה נשמרת עם מספר אירוע אחד, נשלחת לאורן, אריק, שגיב, מוחמד, עוביידה ושירות, ונשלחת אוטומטית ל-Kristin ול-Mr. Dong באנגלית ובסינית. תקלה חוזרת מסומנת HIGH או CRITICAL ונשארת פתוחה עד Root Cause ופתרון קבוע.</p>
    </form>
</section>
<style>
.mcohome-panel{max-width:920px;margin-inline:auto}.mcohome-quick-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.mcohome-fieldset{border:1px solid #dbe4ef;border-radius:16px;padding:14px;margin:0}.mcohome-fieldset legend{font-weight:800;padding:0 8px}.mcohome-chip-grid,.mcohome-fault-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.mcohome-chip{position:relative}.mcohome-chip input{position:absolute;opacity:0;pointer-events:none}.mcohome-chip span{display:flex;min-height:50px;align-items:center;justify-content:center;text-align:center;border:1px solid #cdd8e6;border-radius:12px;padding:8px;background:#fff;font-weight:700;cursor:pointer}.mcohome-chip input:checked+span{background:#1769aa;color:#fff;border-color:#1769aa}.mcohome-chip--fault span{font-size:14px}.mcohome-media-box{border:2px dashed #a9bdd2;border-radius:16px;padding:18px;background:#f8fbff}.mcohome-media-box input{margin-top:8px}.mcohome-details{border:1px solid #dbe4ef;border-radius:14px;padding:14px}.mcohome-details summary{font-weight:800;cursor:pointer}.mcohome-details[open] summary{margin-bottom:14px}.mcohome-submit{font-size:18px;min-height:54px}.mcohome-result-card{display:grid;gap:8px;border:1px solid #b9dec6;background:#f2fbf5;border-radius:14px;padding:16px;margin-bottom:18px}@media(max-width:720px){.mcohome-quick-grid{grid-template-columns:1fr}.mcohome-chip-grid,.mcohome-fault-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.mcohome-panel{padding:14px}.mcohome-chip span{min-height:56px}}
</style>
<script defer src="<?= portal_h(portal_base_path()) ?>mcohome-app.js?v=2"></script>
<?php portal_page_end();
