...
## נמעני דיווח הוצאה חדש

כל דיווח נשלח לאורן (`oren@i-feel.co.il`), לאלי דנון (`account@i-feel.co.il`) ולעובד המדווח לפי `employee.email` המאומת. כולם מקבלים את פרטי הדיווח והמסמכים המצורפים. כתובות כפולות נשלחות פעם אחת בלבד. המשתנה הישן `EXPENSE_PORTAL_REPORT_RECIPIENTS` אינו מחליף או מרחיב רשימה זו. כשל לנמען אחד אינו מפסיק את הניסיונות לשאר הנמענים; כתובת עובד חסרה או כשל שליחה מונעים דיווח הצלחה מלאה.

## חשבוניות בתיק הרכב

דיווח חדש מסוג טיפול או תיקון שומר שיוך מאומת למספר הרכב. מסמכי המקור נשמרים עם הדיווח ומופיעים אוטומטית גם במסמכי הרכב, ללא העתק כפול. ההורדה מתיק הרכב דורשת הרשאת מנהל או שיוך לרכב. דיווח ללא רכב רשום ומורשה דורש תיקון השיוך לפני שמירה. דיווחים ישנים אינם משויכים אוטומטית ללא בדיקה.
