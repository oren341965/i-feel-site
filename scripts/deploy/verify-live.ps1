...
try {
    ...
    if ($gateBody.IndexOf('action="/mt-law/"', [StringComparison]::Ordinal) -ge 0) {
        throw "unsafe-directory-post-target url=$base/mt-law/"
    }
    Write-Host "OK MT-Law WOW gate content, direct form targets and consent markers"
