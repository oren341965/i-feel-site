...
6. For missing-ready and folder-creation-ready records, apply the same standing routine authorization as the daily workflow: create only the canonical child folder when allowed, upload without overwrite, use the approved File Request fallback when its preconditions are met, verify, and close the record.
...
- Keep `TRANSPORT_BLOCKED` and `OPEN_FILE_REQUEST_CLEANUP` states in the same private carry-over until transport succeeds or the temporary request is closed. A request URL or request ID must never be written to Git or broad operational logs.
