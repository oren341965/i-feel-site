...
Keep Maya's work inbox small, classified and actionable without losing customer correspondence. Oren granted standing approval on `2026-08-24` for the bounded inbox organization and routine customer communication defined below. Oren additionally granted standing approval on `2026-09-05` for verified bounce correction and resend, and on `2026-09-06` for bounded hard-bounce cleanup after professional direct mail: Maya may verify a replacement address from authoritative I Feel records, direct correspondence or a current official public source; update the exact Monday professional-contact email and resend once when the match is strong; or remove the exact professional-mailing contact when no verified replacement exists. After verified handling, Maya may move only the bounce notification to Gmail Trash. Everything outside these bounded scopes remains draft-only.
...
5. For bounces, identify the failed recipient, the original sent message and the exact address failure. Then run the verified bounce-handling workflow below. Correct and resend only on a strong match. Remove a contact without a replacement only when the failed message was part of the authorized professional direct-mail cycle and the exact record belongs to board `3040781819`; other unresolved bounces return `NEEDS_OREN`.
...
## Verified bounce correction, cleanup and resend

This workflow is a narrow standing authorization for permanent invalid-recipient delivery failures. It does not authorize general contact enrichment, cold outreach or arbitrary CRM cleanup. The delete-and-Trash path applies only to the activated professional direct-mail workflow; the maturity-0 scheduled pass remains report-only.
...
2. Match the recipient to exactly one existing person/company using the original message context and authoritative I Feel records. Search the matched Monday contact record, recent direct Gmail correspondence and current public sources. Prefer the person's or company's official website; use name plus phone/company to prevent a same-name mismatch.
3. A replacement address is `STRONGLY_VERIFIED` only when the evidence ties it to the same person, or clearly to the same current office/company role, with at least one stable identity field such as the exact phone number. Do not infer an address by changing spelling, username or domain, and never use a guessed pattern such as `firstname@company.com`.
4. When exactly one `STRONGLY_VERIFIED` replacement exists, update only the `Email` field of the exact matched Monday contact. Preserve the item identity, role, phone, owner, status, notes, board relations and all other fields.
5. When no verified replacement exists, delete the exact contact item only if all of these are true: the failed message was authorized professional direct mail; the item is on board `3040781819`; the person is matched unambiguously by the bounced address plus name, phone or company; and a fresh read confirms the item still contains the failed address or that the address was cleared by the same bounce-cleanup run. Check duplicates and remove each duplicate for the same person only when the same evidence proves it. Never delete a sales, project, service or customer item, a company master record for a different valid contact, or an ambiguous identity. Those cases return `NEEDS_OREN`.
6. For a corrected address, resend the same original business message once. Preserve the original subject and business body; do not add new claims, pricing, commitments or marketing content. Remove only the invalid recipient address. Preserve other valid recipients/CCs when they were part of the original message and remain appropriate. A deleted contact receives no resend.
7. Verify every Monday write with a fresh read: the corrected email must be present, or every intended deleted item ID and failed address must be absent. Verify Gmail Sent for a corrected resend. If any required write or verification fails, report `PARTIAL_BOUNCE_HANDLING` and do not silently claim completion.
8. After the Monday outcome and any required corrected resend are verified, move only the corresponding bounce notification to Gmail Trash and verify it is absent outside Trash. Keep the original sent message. Never permanently delete mail. If handling is partial or ambiguous, retain the bounce in the inbox with the `bounce` label.
9. Deduplicate by original sent-message ID plus failed recipient. Never perform more than one automatic correction/resend attempt for the same bounced recipient. A second bounce on the corrected address returns `NEEDS_OREN`.
10. Record only bounded operational evidence in logs: original recipient hash, corrected recipient hash when applicable, Monday item ID, verification source type, correction/deletion timestamp, send verification status and bounce-to-Trash status. Do not store message body text or unnecessary personal data in shared logs.
...
6. Return one structured `MAYA_SALES_TASK_RESULT`. Use `MAYA_EXECUTED` after the bounded action; use `WAITING_FOR_CUSTOMER` only with `next_action` and `next_treatment_date`; use `RESPONSE_RECEIVED_AND_MONDAY_UPDATED` only as a claimed outcome for manager verification. Maya does not update Monday except for the exact verified bounce-handling writes on professional-contact board `3040781819` defined above; the manager applies other task outcomes and verifies a fresh read-back.
...
The verified bounce-handling workflow above is a separate narrow send/write authorization. It may update one matched Monday professional-contact email and resend the same previously sent business message once, or delete the exact professional-mailing contact when no verified replacement exists, and then Trash only the handled bounce notification. It does not authorize broader outreach, new content or deletion from operational boards.
...
- Never permanently delete mail. Outside the verified professional direct-mail bounce workflow, moving a message to Trash requires a separate explicit request for the inspected messages.
...
- Never change Monday, WhatsApp, Calendar, contacts, advertisements or budgets from this skill except for the exact matched `Email` correction or exact professional-contact deletion on board `3040781819` in the verified bounce-handling workflow. Do not alter another Monday field, delete from another board, change status/owner, or treat a bounce as permission for general CRM cleanup.
- The scheduled maturity-0 pass remains report-only and may not perform bounce correction, contact deletion, resend, Trash or any other write. A write-enabled manual run or activated professional-content cycle may perform the verified bounce-handling workflow only when every evidence gate passes. It must not permanently delete mail, write to the Vault or Bus, or edit local files.
...
- bounce handling: corrected Monday email fields, deleted professional-contact items, corrected resends verified, bounce notifications moved to Trash, ambiguous/unresolved bounces and partial outcomes;
