...
7. אם זה סרטון פרויקט, סווג אותו כ־`BMS`, `MULTIFAMILY` (בנייה רוויה) או `VILLA`, והעבר ל־`ai-sales-manager`
   את סוג הפרויקט ואת כתובת דף האתר החי (או כתובת YouTube אם אין דף מתאים). מנהל המכירות יכין למאיה
   תכנית לעדכון שבועי קצר לקהל המתאים; `video-add` עצמו אינו בונה רשימת נמענים ואינו שולח דוא"ל.
