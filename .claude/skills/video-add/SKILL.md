...
## שלב 2 — העלאה ל-YouTube  ⚠️ דורשת אישור מפורש

YouTube Data API v3 מחובר לערוץ `@ifeelsmarthome` דרך OAuth מקומי. קובצי הלקוח וה-token
נשמרים מחוץ לריפו, והנתיבים שלהם מוגדרים ב-`.env.local`. ראה `docs/youtube-api-uploader.md`.

1. הרץ `npm run youtube:verify` ואמת שהתקבל הערוץ `UC7nVAqqWJiFhp-EshK2vmMA`.
2. הצג לאורן את שם הקובץ, הכותרת, התיאור, התגיות והנראות (`private`/`unlisted`/`public`).
3. רק אחרי אישור מפורש צור metadata JSON מקומי והריץ:
   `npm run youtube:upload -- --file <video> --metadata <metadata.json> --approve-upload`.
4. העלאה פומבית דורשת בנוסף `--approve-public`.
5. קח את `videoId` מהפלט והמשך להטמעה באתר.
...
- אל תעלה/תפרסם/תשנה ב-YouTube בלי אישור מפורש. השתמש רק בכלי API המוגן ובערוץ המאומת.
