...
When both `connected` and `liveVerified` are true in the machine-local runtime configuration, `morning-run.mjs` invokes this connector and exposes the live read under `metaAdsReadOnly`. Lead-form retrieval also requires `leadFormsReadOnly=true`, an explicit numeric `pageId`, and Page/form permissions on the existing protected credential. It verifies the Page through `/me/accounts`, derives the Page access token in memory without returning or logging it, uses that Page token for `/leadgen_forms` and `/leads`, reads only `id,created_time` from leads, then returns aggregate counts; it never requests `field_data` or exposes a person's lead fields.

Treat `ads_management` and `pages_manage_ads` as elevated credential scopes even when this connector itself is GET-only. Never add, regenerate, replace, or broaden the protected credential automatically. If Meta requires either scope for `/leadgen_forms`, report the missing permission signals and wait for Oren's explicit credential-scope approval, or route lead ingestion through a separately approved least-privilege export.
