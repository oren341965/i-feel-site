...
- For a production send, require the Assignment's immutable `action_authorization`, validate that it is current and issued by Oren, resolve the exact approved text locally, and match its SHA-256 before touching Send. Produce a pre-send preview proving the same `task_id`, verified direct `CUSTOMER` recipient, channel and content hash. The channel adapter must use `task_id` as its idempotency key and return a post-send receipt with the same values plus verified appearance in the direct conversation. Missing or mismatched authorization, preview or receipt is `BLOCKED`.
...
- For every sales follow-up candidate, reuse the parent `ai-sales-manager` `SALES_ELIGIBILITY_FILTER` before drafting or sending. Exclude Projects/Service handoffs, ended sales, closed deals/open customer files, future `timeline` follow-ups, and same-cycle records without newer verified evidence. A reached follow-up date or new verified Gmail, Calendar or Monday-update evidence makes the record reviewable again; authoritative stage/evidence overrides stale `ליד חדש`.
