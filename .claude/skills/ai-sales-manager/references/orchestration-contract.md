...
The existing `maya-whatsapp` scheduled task runs every five minutes and evaluates a daily gate at 15:00 in `Asia/Jerusalem`. It reads the current month tab of spreadsheet `1_r2WSYvpUWlBRz_6yX5Yqr5KKUAOVNpte6CoZYttdII`, finds only the current date block and sends one consolidated direct request for photos plus a short factual field note to each verified technician with field assignments. Before sending, it reads the verified recent direct WhatsApp conversation as the duplicate ledger; if that read is unavailable, it does not send. The unattended task never invokes `Edit`, writes files, downloads media, mutates the Vault, Bus, spreadsheet or Monday, or creates a second scheduler.
