...
8. Run the installed `test-live-readonly-preflight.ps1 -ConfirmMayaWorkstation` helper. It must return `LOCAL_PREFLIGHT_PASS`, matching current-manifest and installed WhatsApp hashes, `DORMANT_PRODUCTION_DISABLED`, zero Bus delta, current commissioning evidence that the Windows email task is disabled, and a successful network-free Management identity dry run. It must not call Windows Task Scheduler or write to the Bus.
9. After separate live-read gates and action-specific approval, production customer work must use `maya-task-production-runner.mjs prepare` followed by `complete`; no installed caller may use the bridge's isolated processor as a production shortcut.
10. Treat `PASS_ISOLATED` and `LOCAL_PREFLIGHT_PASS` as protocol and local-installation evidence only. Read-only Gmail, WhatsApp and Monday connector checks still run through Maya's Codex session. Real readiness also requires fresh authenticated Management evidence and action-specific approval.
11. Continue only when the newest result is `INSTALLED_PAUSED`, all four Codex Skill hashes, both Maya task contract hashes and all four task-runtime hashes match, `managementCredentialsProvisioned=true`, `runtimeLocks=0`, `claudeRequired=false`, and all external-action counters are zero.
12. Browser/account, Gmail, Instagram, Facebook, WhatsApp and Management System identity smoke tests are a later, separately approved gate. Scheduler activation remains a separate approval after those checks.
