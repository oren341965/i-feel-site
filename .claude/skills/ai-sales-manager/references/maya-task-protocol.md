...
The deterministic implementation is `scripts/maya-vault-bridge.mjs`; the installed production caller is `scripts/maya-task-production-runner.mjs`; the JSON contract is `runtime/bus-message.schema.json`. Production is two-phase: `prepare` validates fresh live evidence, a matching SHA-256 direct-recipient identity across Monday/channel/preview, and exact-content preview before any action; `complete` accepts only a matching verified direct-channel receipt. A preparation expires after five minutes and is deliberately retained as a blocker until reconciled if completion does not occur.
