...
4. Accept only `INSTALLED_PAUSED` with four of four Codex Skill hashes, both task-contract hashes, all three deterministic task-runtime hashes, zero runtime locks, zero scheduler activations, zero sends, zero Monday writes and zero deletions.
...
8. Read the bounded result from Oren's computer, then send one sanitized Host check-in and one `REPORT_ONLY` worker run. Do not activate the scheduler until both are accepted and Oren separately approves activation.
