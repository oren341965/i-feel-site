...
3. Run `INSTALL_CURRENT.ps1 -ConfirmMayaWorkstation`. The installer copies only the three Maya workers, the telemetry adapter and the role-scoped `AGENTS.md` into `%USERPROFILE%\.codex`.
4. Accept only `INSTALLED_PAUSED` with four of four Codex Skill hashes, both task-contract hashes, all four deterministic task-runtime hashes, zero runtime locks, zero scheduler activations, zero sends, zero Monday writes and zero deletions.
...
7. Run the installed isolated task bridge smoke: `node C:\ifeel-maya\jobs\maya-task-e2e-smoke.mjs --config C:\ifeel-maya\config\config.json`. Require Assignment received, ACK returned, Result returned, duplicate protection, and zero external actions. This test deliberately leaves `READY_FOR_REAL_TASKS=NO` until live read-only identities and connectors are verified.
8. Verify that `C:\ifeel-maya\jobs\maya-task-production-runner.mjs` is present and hash-matched. Real customer actions are permitted only through its `prepare` and `complete` phases after their separate live and approval gates pass.
9. Read the bounded result from Oren's computer, then send one sanitized Host check-in and one `REPORT_ONLY` worker run. Do not activate the scheduler until both are accepted and Oren separately approves activation.
