...
- For the complete canonical Skill selection made for Maya, read [references/maya-codex-skill-review.md](references/maya-codex-skill-review.md). Do not replace the reviewed three-Skill bundle with a copy of every manager and specialist.
...
- Maya commissioning installs the two canonical Maya workers plus the telemetry adapter into Codex only. It never installs the parent manager on Maya, never activates a scheduler, never requires Claude, and never treats the Vault as executable source truth.
