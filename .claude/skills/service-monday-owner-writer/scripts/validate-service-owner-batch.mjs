...
const PRIVATE_ROOT = resolve(REPO, '.ai-manager-data/service/tmp');
...
function privatePath(value, label) {
  ...
  if (rel.startsWith('..') || isAbsolute(rel)) fail(`${label} must be inside .ai-manager-data/service/tmp`);
  return absolute;
}
...
export function validateServiceOwnerBatch({ preview, approval, readback, now = new Date().toISOString() }) {
  ...
  if (scope.maxItems !== MAX_ITEMS || scope.preserveExisting !== true) fail('approval safety flags are invalid');
  ...
  for (const proposal of proposals) {
    ...
    const currentKeys = current.map((entry) => `${entry.kind}:${entry.id}`);
    const proposedKeys = proposed.map((entry) => `${entry.kind}:${entry.id}`);
    if (!currentKeys.every((key, index) => proposedKeys[index] === key)) fail(`proposal ${itemId} does not preserve existing ownership order`);
    const added = proposed.slice(current.length);
    if (added.length !== 1 || added[0].kind !== 'person' || added[0].id !== addPersonId) fail(`proposal ${itemId} adds an unauthorized identity`);
  ...
  const batchFingerprint = createHash('sha256').update(canonical({ boardId: BOARD_ID, columnId: COLUMN_ID, itemIds: proposalIds, addPersonId, updates })).digest('hex');
  return {
    schemaVersion: 1,
    ...
    updates,
    rollbacks,
    safeguards: { singleBatchCall: true, postWriteReadbackRequired: true, retryBusinessWrite: false, customerDataIncluded: false },
  };
}
...
async function main() {
  ...
  const plan = validateServiceOwnerBatch({ preview, approval, readback, now: get('--now') ?? new Date().toISOString() });
