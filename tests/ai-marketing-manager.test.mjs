...
test('AI Marketing Manager optimizes qualified pipeline and preserves worker authority', async () => {
  ...
  const registration = JSON.parse(await readFile(resolve(REPO, '.claude/skills/ai-marketing-manager/references/management-registration.json'), 'utf8'));
  ...
  assert.equal(registration.capabilities[0].slug, 'ai-marketing-manager');
  assert.notEqual(registration.capabilities[0].permissions.write, 'allowed');
  assert.notEqual(registration.capabilities[0].permissions.budget, 'allowed');
});
